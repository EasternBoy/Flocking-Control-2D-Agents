...
po = zeros(2,4,N);
for i = 1:N
    po(:,:,i)  = [0.4 0; 0 -0.2; -0.4 0; 0 0.2]'; % diamond shape
end
S    = [0 -1;1 0];
...
axis([-5   12.5   -7.5   8]);
set(gca,'FontSize',14,'Position',[0.06 0.6 0.43 0.4]);box on; grid on;
...
axis([-5   12.5   -7.5   8]);
set(gca,'FontSize',14,'Position',[0.56 0.6 0.43 0.4]);box on; grid on;
...
axis([-5   12.5   -7.5   8]);
set(gca,'FontSize',14,'Position',[0.06 0.1 0.43 0.4]);
...
axis([-5   12.5   -7.5   8]);
set(gca,'FontSize',14,'Position',[0.56 0.1 0.43 0.4]);
...
fd = axes('Position',[0.09 0.075 0.88 0.17]);
...
fc = axes('Position',[0.09 0.32 0.88 0.17]);
...
fb = axes('Position',[0.09 0.565 0.88 0.17]);
plot((1:Ns)*dT, z_traj(1,1:Ns,i),'b','LineWidth',1); hold on; grid on;
plot((1:Ns)*dT, z_traj(2,1:Ns,i),'r','LineWidth',1);
plot((1:Ns)*dT, dqod(1,:) - (q_traj(2,1:Ns,i)- qod(2,:))*(-Ome),'m--','LineWidth',1);
plot((1:Ns)*dT, dqod(2,:) + (q_traj(1,1:Ns,i)- qod(1,:))*(-Ome),'c--','LineWidth',1);
axis([0 T -0.5 0.82])
legend('$v_{1x}(t)$','$v_{1y}(t)$',...
    '$\dot{x}_{od}(t)-(y_1(t)-y_{od}(t))\omega_1(t)$',...
    '$\dot{y}_{od}(t)+(x_1(t)-x_{od}(t))\omega_1(t)$',...
...
% subplot(4,1,1);
fa = axes('Position',[0.09 0.81 0.88 0.17]);
